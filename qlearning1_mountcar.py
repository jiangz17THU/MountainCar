...
EPISODES = 7000   #训练次数
SHOW_EVERY = 200
Q_TABLE_LEN = 20   #Qtable长度

#set up q table 20*20*3
DISCRETE_SIZE = [Q_TABLE_LEN] * len(env.observation_space.high)
...
def choose_epilon_greedy_action(state, epsilon): #贪婪策略的实现方式，保证智能体前期能够大胆探索，后期能较快收敛到最优策略
...
for episode in range(EPISODES):
    ...
    while not done:
        action = choose_epilon_greedy_action(state, epsilon) #如果没有到达目的地，按照规定的贪婪策略采取行动

        next_state, reward, done, _ = env.step(action)    #环境根据行动反馈下一步状态，奖励值以及是否到达终点

        ep_reward += reward  #累计奖励值
